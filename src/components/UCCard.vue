...
import { Calendar, Users, BookOpen, ExternalLink, FileText, Eye } from "lucide-vue-next";
...
<template>
  <Card class="h-full flex flex-col hover:shadow-lg transition-shadow duration-200">
    <CardHeader class="min-h-[120px] flex flex-col justify-between">
      <div class="flex items-start justify-between gap-2">
        <a :href="`/uc/${uc.slug}`">
          <CardTitle
            class="text-lg font-bold text-balance hover:text-primary transition-colors cursor-pointer"
          >
            {{ uc.nome }}
          </CardTitle>
        </a>
      ...
      <div class="flex items-center justify-between">
        <CardDescription class="text-pretty">
          Clica nas secções abaixo para ver mais detalhes
        </CardDescription>
        <a :href="`/uc/${uc.slug}`">
          <Button variant="outline" size="sm" class="ml-2 bg-transparent">
    ...
    <CardContent class="flex-1 flex flex-col">
      <Accordion type="single" collapsible class="w-full">
        <AccordionItem value="avaliacoes">
          ...
          <AccordionContent>
            <div class="space-y-3">
              <div v-if="!uc.avaliacoes || uc.avaliacoes.length === 0" class="p-3 bg-muted rounded-lg text-sm text-muted-foreground">
                Ainda sem informação.
              </div>
              <div
                v-else
                v-for="(avaliacao, index) in uc.avaliacoes"
                :key="index"
        ...
        <AccordionItem value="recursos">
          ...
          <AccordionContent>
            <div class="space-y-2">
              <div v-if="!uc.recursos || uc.recursos.length === 0" class="p-3 bg-muted rounded-lg text-sm text-muted-foreground">
                Ainda sem informação.
              </div>
              <a
                v-else
                v-for="(recurso, index) in uc.recursos"
                :key="index"
                :href="recurso"
                target="_blank"
        ...
        <AccordionItem value="docentes">
          ...
          <AccordionContent>
            <div class="space-y-2">
              <div v-if="!uc.docentes || uc.docentes.length === 0" class="p-3 bg-muted rounded-lg text-sm text-muted-foreground">
                Ainda sem informação.
              </div>
              <div
                v-else
                v-for="(docente, index) in uc.docentes"
                :key="index"
                class="text-sm p-2 bg-muted rounded-lg"
              >
                {{ docente }}
              </div>
            </div>
